...
```{r}
time_spent <- time_spent |> 
  mutate(time_spent_hours = round(time_spent/ 60 , 3))
```
...
-   The survey data frame has variables like student_id, subject, semester, section, which are also in the gradebook data frame.
-   The survey can be combined with the gradebook to see some relationships between variables from both data frames.
...
-   There is a missing value because df5 has 3 cases and df6 has 4 cases.
-   Two data frames became one, new combined data frame.
...
```{r}
# join together the gradebook and log_wrangled
joined_data2 <- full_join(gradebook, time_spent, by=c("student_id", "subject", "semester", "section"))
...
-   The gradebook data frame has 717 obsevations of 7 variables and the time_spent data frame has 716 observations of 9 variables.
-   The new data frame, joined_data has 830 observations of 12 variables.
-   There some missing values in the new data frame, joined data.
...
-   The data_to_explore has 943 observations of 34 variables.
-   The skimr function summarized how many missing values are in each variable.
-   There are three different tables. How were they formed?
...
```{r}
ggplot(data_to_explore) +
  geom_histogram(aes(x = time_spent_hours), bins = 15)

ggplot(data_to_explore) +
  geom_histogram(aes(x = time_spent_hours), bins = 60)
```
...
-   I think there are reasons bins = 30 is the default. Maybe, is it because many people notice from their analyses that bins=30 is most effective to interpret most of the cases? However, it really depends on the data. In this case, I think all three (bins= 15,30, 60) show positive skewness and some outliers ,so any of them can be used if we just want to see the tendency of the time spent.
...
ggplot(data_to_explore) +
  geom_histogram(aes(x = time_spent_hours), color="blue")+
  facet_wrap(~subject)
```

What do you notice about this figure? What questions do you have? Add a note (or a few notes!) below:

-   FrScA histogram looks similar with the time_spent_hours for all subjects.
-   It looks like the number of students who take FrScA is bigger than other subjects.
-   Each subject has different trends on time spend.
...
-   From the scatter plot, there is relationship between time_spent_hours and proportion_earned.
-   I think there is postive relationship between those two variables, but there are outliars on each side.
-   Also, some students get high proportion even though they spent very short time.
-   And number of dots are not enough to represent large number of cases, which means more than one dot would be on a certain point.
...
-   Most students completed works in 50 hours and got higher than 70% of points.
-   There is positive relationship between time spent and proportion of points.
...
```{r}
data_to_explore |> 
  select(proportion_earned, time_spent_hours, total_points_earned, int, val, percomp) |> 
  correlate() |> 
  rearrange() |>
  shave() |>
  fashion()
...
```{r}
m2 <- lm(proportion_earned ~ time_spent_hours + percomp + val, data = data_to_explore)
...
-   The students' earned proportion has positive relationship with perceive to computer. If one unit of perception of computer increase, we can estimate that the earned proportion would 4% increase.

-   The students' earned proportion has positive relationship with time spent. If one spent hour increase, we can estimate that the earned proportion would 0.4 % increase.

-   The 'val' variable has negative relation with the earned proportion but p-value is bigger than 0.05 so the effect is not significant.
