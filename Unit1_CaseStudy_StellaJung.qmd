...
-   I think total time online, total \# discussion meaasges posted, Time spent on assessments, and Time spent on assignments would predict final grade.
...
-   In the dicussion section 4.1, three significant predictor variables are number of forum posting, mail massages sent, and assessment completed. This model has 80.9% accuracy for identifying "at risk" students.
...
-   log-data seperate each variables using "," while the tibble in the console area organized better by colums for each variables.
-   However the tibble only shows first 10 cases from the log-data.
-   I am wondering how the average time-spent would be different for each course.
...
-   In the survey data, there are missing values.
-   There are different number of cases between gradebook and survey. It looks like some wrangling process would be needed.
...
-   Because column AF to AG have notes (green triangles at the corner).
...
```{r}
# YOUR CODE HERE
separate(df2, course_variable, c("year", "semester", "course"))
```
